...
def export_data():
    ...
    table=pandas.DataFrame(newlist,columns=['Id','Name','Phone','Email','Address','Gender','D.O.B','Added Date','Added Time'])
...
def add_data():
    ...
    if idEntry.get()=='' or nameEntry.get()=='' or phoneEntry.get()=='' or emailEntry.get()=='' or addressEntry.get()=='' or genderEntry.get()=='' or dobEntry.get()=='':
        ...
        try:
            ...
            messagebox.showerror('Error','Id cannot be same', parent=screen)
            return
...
def connect_database():
    def connect():
        global mycursor,con
        try:
            con=pymysql.connect(host='localhost',user='root',password='Nirmal07@1999')
            mycursor=con.cursor()
        except:
        ...
        try:
            ...
            query='create table student(id int not null primary key,name varchar(30),phone varchar(10),'\
                  'email varchar(30),address varchar(100),gender varchar(20),dob varchar(30),date varchar(50),time varchar(50))'
            mycursor.execute(query)
        except:
            query='use studentmanagementsystem'
            mycursor.execute(query)
    ...
    hostnameLabel=Label(connectWindow,text='Host Name', font=('aria',20,'bold'))
    hostnameLabel.grid(row=0,column=0,padx=20)

    hostEntry=Entry(connectWindow,font=('roman',15,'bold'),bd=2)
    hostEntry.grid(row=0,column=1,padx=40,pady=20)

    usernameLabel=Label(connectWindow,text='User Name', font=('aria',20,'bold'))
    usernameLabel.grid(row=1,column=0,padx=20)

    usernameEntry=Entry(connectWindow,font=('roman',15,'bold'),bd=2)
    usernameEntry.grid(row=1,column=1,padx=40,pady=20)

    passwordLabel = Label(connectWindow, text='Password', font=('aria', 20, 'bold'))
...
root.title('Student Management Sytem')
...
studentTable=ttk.Treeview(rightFrame,columns=('Id','Name','Mobile No','Email',
                                 'Address','Gender','D.O.B','Added Date','Added Time'),
